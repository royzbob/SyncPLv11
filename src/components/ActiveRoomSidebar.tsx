...
export default function ActiveRoomSidebar({
  ...
  const [sidebarSection, setSidebarSection] = useState<"all" | "channels" | "views">("all");
  ...
  return (
    <aside className="w-full md:w-60 bg-[#121417] border-r border-[#2A2D31] flex flex-col shrink-0 z-20 relative h-full min-h-0 select-none text-xs">
      ...
      {/* Brand Header */}
      <div className="p-3.5 border-b border-[#2A2D31] bg-[#08090A]/40 space-y-2 shrink-0">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onCopyRoomCode}
            className="p-1.5 bg-[#1E2023] hover:bg-[#2A2D31] border border-[#2A2D31] text-indigo-400 rounded-lg transition cursor-pointer shrink-0 shadow-sm"
            title="Copy invite code"
          >
      ...
      {/* Quick Section Switcher for Mobile & Desktop Navigation */}
      <div className="px-3 py-2 bg-[#0A0C0E] border-b border-[#2A2D31]/60 shrink-0">
        <div className="grid grid-cols-3 gap-1 bg-[#141619] p-1 rounded-lg border border-[#2A2D31]/60 text-[10px] font-bold">
          <button
            type="button"
            onClick={() => setSidebarSection("all")}
            className={`py-1.5 px-1 rounded text-center transition cursor-pointer ${
              sidebarSection === "all"
                  ...
                  <div className="flex items-center gap-1">
                    {textChans.length > 1 && onMoveChannel && (
                      <button
                        ...
                        className={`transition cursor-pointer p-1 rounded ${
                          isReorderingText
                    ...
                    <button
                      ...
                      className="text-gray-500 hover:text-white transition cursor-pointer p-1 rounded hover:bg-[#1E2023]"
                ...
                {textChans.map((chan, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group relative flex items-center justify-between px-2.5 py-2 min-h-[42px] sm:min-h-[38px] rounded-lg text-xs font-semibold transition-all duration-150 border ${
                    ...
                    >
                      ...
                      >
                        ...
                        <span className={`font-black text-sm shrink-0 ${isSelected ? "text-indigo-400" : "text-[#72767D]"}`}>#</span>
            ...
            <div>
              <div className="flex items-center justify-between px-2 mb-1.5">
                <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest flex items-center gap-1">
                  <Radio className="w-2.5 h-2.5 text-emerald-400" /> Voice & Call Rooms
                </span>
                {isCreatorOrMod && (
                  <div className="flex items-center gap-1">
              ...
              <div className="space-y-1.5">
                {voiceChans.map((chan, idx) => {
                  ...
                  const chanUsers = voiceUsers.filter((u) => u.channel === chan.name);
                  const count = chanUsers.length;
                  const isAi = chan.isAiRoom || chan.name.toLowerCase().includes("ai") || chan.name.toLowerCase().includes("advisor");
                  const isFirst = idx === 0;
                  ...
                  return (
                    <div key={chan.id} className="space-y-0.5">
                      <div
                        ...
                        className={`group relative flex items-center justify-between px-2.5 py-2 min-h-[44px] sm:min-h-[38px] rounded-lg text-xs font-semibold transition-all duration-150 border ${
                      ...
                      >
                        ...
                        >
                          ...
                          {isAi ? (
                            <Bot className={`w-4 h-4 shrink-0 ${isConnected ? "animate-pulse text-indigo-400" : "text-gray-500"}`} />
                          ) : (
                            <Volume2 className={`w-4 h-4 shrink-0 ${isConnected ? "text-emerald-400 animate-pulse" : "text-gray-500"}`} />
                          )}
                          <span className="truncate font-bold">{chan.name}</span>
                          {chan.pin && (
                            <span title="PIN Protected">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          {/* Quick Join Call / Active Status Button */}
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                              onToggleVoiceRoom(chan.name);
                            }}
                            className={`text-[10px] font-bold px-2 py-0.5 rounded transition cursor-pointer ${
                              isConnected
                                ...
                                : "bg-[#1E2023] hover:bg-emerald-500/20 text-gray-300 hover:text-emerald-400 border border-[#2A2D31]/40"
                            }`}
                            title={isConnected ? "Disconnect from voice call" : "Join voice call"}
                          >
                            {isConnected ? "Joined" : "Join"}
                          </button>

                          <span className="text-[10px] bg-[#1E2023] px-1.5 py-0.5 rounded-full text-gray-400 font-mono border border-[#2A2D31]/30 shrink-0">
                            {count}
                          </span>
                        </div>
                      </div>
